...
int LuaObjcBridge::callObjcStaticMethod(lua_State *L)
{
    ...
    @try {
        ...
        if (hasArguments)
        {
            NSObject *obj = (NSObject *)LuaObjcBridge::copyToObjc(L);
            if ([obj isKindOfClass:[NSArray class]]) {
                NSArray *array = (NSArray *)obj;
                NSString *methodName_ = [NSString stringWithCString:methodName encoding:NSUTF8StringEncoding];
                NSArray *argNames = [methodName_ componentsSeparatedByString:@":"];
                NSUInteger argCount = [argNames count];
...
void* LuaObjcBridge::copyToObjc(lua_State *L)
{
    switch (lua_type(L, -1))
    {
        case LUA_TNIL:
        ...
        case LUA_TTABLE:
        {
            ...
            if (bDictionary) {
                return LuaObjcBridge::checkDictionary(L);
            }
            else {
                return LuaObjcBridge::checkArray(L);
            }
        }
    }
    return nil;
...
void* LuaObjcBridge::checkArray(lua_State *L)
{
    NSMutableArray *array = [NSMutableArray array];
    lua_pushnil(L);  /* first key */
    while (lua_next(L, -2)) {
        int index = lua_tonumber(L, -2) - 1;
        NSObject *value = (NSObject*)LuaObjcBridge::copyToObjc(L);
...
void* LuaObjcBridge::checkDictionary(lua_State *L)
{
